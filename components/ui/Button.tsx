...
const variants: Record<Variant, string> = {
  ...
  primary:
    'font-semibold tracking-[0.08em] uppercase text-[#0A0C0F] ' +
    'bg-[#FAF8F5] border border-[#FAF8F5]/80 ' +
  ...
  outline:
    ...
    'bg-transparent text-[#FAF8F5] border-2 border-[#FAF8F5] ' +
    'hover:bg-[#FAF8F5] hover:text-[#0A0C0F]',

  // Light — for dark section CTA
  light:
    'font-medium tracking-[0.08em] uppercase ' +
    'bg-transparent text-[#FAF8F5] border-2 border-[rgba(242,237,228,0.4)] ' +
    'hover:bg-[#FAF8F5] hover:text-[#0A0C0F]',

  // Tertiary — text-only, no border, no uppercase, no tracking
  tertiary:
    'font-normal text-sm ' +
    'bg-transparent border-none text-[#C3C0BB] ' +
    'hover:text-[#FAF8F5] hover:underline underline-offset-4',
}
...
export function Button({
  ...
  if (!href) {
    const buttonEl = (
      <button
        type={type}
        onClick={onClick}
        className={`${classes} cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-[#C78B28] focus-visible:ring-offset-2 focus-visible:ring-offset-[#0A0C0F]`}
      >
        {content}
      </button>
    )
  ...
  const linkEl = (
    <Link
      ...
      className={`${classes} focus:outline-none focus-visible:ring-2 focus-visible:ring-[#C78B28] focus-visible:ring-offset-2 focus-visible:ring-offset-[#0A0C0F]`}
    >
      {content}
    </Link>
  )
